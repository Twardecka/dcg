all:
	...
	# GPU 4
	CUDA_VISIBLE_DEVICES=0 python src/main.py --config=dcg --env-config=gymma with env_args.key="pz-mpe-simple-tag-v3" seed=7 use_cuda=True &
	CUDA_VISIBLE_DEVICES=0 python src/main.py --config=dcg --env-config=gymma with env_args.key="pz-mpe-simple-tag-v3" seed=8 use_cuda=True &
